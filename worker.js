...
async function readMessages(env, scope, limit = 100) { const keys = (await env.LCC_KV.get(`index:${scope}`, 'json')) || []; const selected = keys.slice(-limit); const values = await Promise.all(selected.map(key => env.LCC_KV.get(key, 'json'))); return values.filter(Boolean); }
  ...
  async fetch(request, env) {
    ...
    if (parts.join('/') === 'api/v1/channels/central/messages') {
      ...
      if (request.method === 'POST') {
        ...
        const text = cleanText(payload?.text); const author = cleanAccount(payload?.author);
        if (!text || !author) return json({ error: 'text and author are required.' }, 400);
        const message = { id: id(), text, author, createdAt: Date.now(), channel: 'central' };
    ...
    if (parts[0] === 'api' && parts[1] === 'v1' && parts[2] === 'dms' && parts[3]) {
      ...
      if (request.method === 'POST') {
        ...
        const message = { id: id(), text, author, createdAt: Date.now(), recipientId: partnerId };
